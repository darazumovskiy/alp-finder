...
- Выход `agent/notes/<дата>.json`: `ok`, `model`, `usage{in,out}`, `stop_reason`, `images[]` (подписи),
  `raw` (ответ целиком), `attempts[]`, поля схемы; при отказе/не по форме — `ok:false` и `error`, страница показывает причину.
- **Гвард формы (с 03.09).** Причина сбоев 29.08, 30.08 и 03.09: модель заканчивала ход без закрывающей `}`,
  regex-разбор не находил объект, заметка сохранялась с «ответ не по схеме». Теперь: (1) схема `SCHEMA` уходит в
  API как structured output (`output_config.format = json_schema`) — синтаксис и набор полей гарантированы;
  (2) поверх — `validate()`: дата, непустые тексты, перечисления `trend`/`confidence`, `flags` — список строк;
  (3) не по форме → до `MAX_ATTEMPTS = 3` повторов той же моделью; (4) если всё равно плохо, а за дату уже есть
  заметка с `ok:true` — прежняя остаётся, плохая не записывается; (5) код выхода 5 → шаг `agent` в статус-баре
  красный (общий «ok» прогона не снимает). `parse_json()` терпим к тексту вокруг объекта.
