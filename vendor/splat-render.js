...
// поле зрения задаётся ?fov= (градусы по вертикали, по умолчанию 60):
// зашитые в демо fx/fy — от чужой камеры, наши сцены сняты телевиком
{
    const fovDeg = +(new URLSearchParams(location.search).get("fov") || 60);
    const f = (innerHeight * 0.5) / Math.tan(((fovDeg / 2) * Math.PI) / 180);
    camera.fx = camera.fy = f;
...
async function main() {
    ...
    // только свои файлы рядом со страницей — никаких внешних хостов.
    ...
    if (urlParam.includes(",")) {
        ...
        url = new URL(urlParam, location.href);
    }
    ...
    const frame = (now) => {
        ...
        if (activeKeys.includes("ArrowRight"))
            inv = translate4(inv, 0.03, 0, 0);
        // WASD — полёт как в «Полёте 3D» (у оригинала было вращение —
        // пользователи ожидают движение), Q/E — крен
        const fly = shiftKey ? 0.30 : 0.10;
        if (activeKeys.includes("KeyW")) inv = translate4(inv, 0, 0, fly);
        if (activeKeys.includes("KeyS")) inv = translate4(inv, 0, 0, -fly);
        if (activeKeys.includes("KeyA")) inv = translate4(inv, -fly * 0.6, 0, 0);
        if (activeKeys.includes("KeyD")) inv = translate4(inv, fly * 0.6, 0, 0);
